...
### 新增功能
- **对外开放 API（PRD-00008）**：新增可供外部系统调用的邮件查询、原始内容与验证码/链接提取能力，并补全 OpenAPI 文档与契约测试
- **公网安全守卫层（P1）**：支持“公网模式”开关、IP 白名单（含 CIDR）与按 IP 的每分钟限流；并支持禁用高风险端点（`raw` / `wait-message`）
- **多 External API Key 管理**：支持多 Key 创建/启停/删除与邮箱白名单作用域控制；Key 加密存储并脱敏回显
- **wait-message 异步探测（P2）**：新增 probe 缓存与后台轮询能力，支持异步返回 `probe_id` 并通过 probe 端点查询结果
- **验证码提取参数化增强**：支持按长度范围/正则/来源（subject/html/body）等选项提取验证码与链接

### 修复
- **BUG-00017**：低置信度验证码/链接提取结果不再误返回错误值，改为返回 404（提升调用可靠性）
- 若干 UI 回归修复：登出/复制/图标等细节问题修复

### 重要变更
- **数据库 Schema 升级：v5 → v10**（启动自动迁移）：新增外部 API Key、限流、异步 probe、调用统计等相关表；建议发布前备份数据库文件
- **对外 API 行为调整**：在启用公网模式后，非白名单 IP 访问将被拒绝；超过限流阈值将返回 429；被禁用的端点将返回 403

### 测试/验证
- 单元测试：`python -m unittest discover -s tests -v`（Ran 378 tests in 50.878s，全部通过）
